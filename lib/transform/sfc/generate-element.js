...
function solveClass(attrs, state) {
  ...
  if (hasClass && hasVclass) {
    ...
    attrVal = t.jSXExpressionContainer(t.templateLiteral([t.templateElement({
      raw: '',
      cooked: ''
    }), t.templateElement({
      raw: ' ',
      ...
      raw: '',
      cooked: ''
    }, true)], [isUseCssModule ? t.memberExpression(t.identifier('styles'), t.stringLiteral(classItem.value), true) : t.stringLiteral(classItem.value), (0, _directives.handleExpression)(state, vClassItem.value)]));
  } else if (hasClass) {
